...
const BookSingleCard = ({ book }) => {
    ...
    return (
        <div key={book._id} className='border-2 border-gray-500 rounded-lg px-4 py-2 m-4 relative hover:shadow-xl bg-slate-700'>
            <h2 className='absolute top-1 right-2 px-4 py-1 bg-red-300 rounded-lg'>{book.publishYear}</h2>
            <h4 className='my-2 text-neutral-400'>{book._id}</h4>
            <div className='flex justify-start books-center gap-x-2'>
                <PiBookOpenTextLight className='text-red-500 text-2xl' />
                <h2 className='my-1 text-neutral-50'>{book.title}</h2>
            </div>
            <div className='flex justify-start books-center gap-x-2'>
                <BiUserCircle className='text-red-500 text-2xl' />
                <h2 className='my-1 text-neutral-50'>{book.author}</h2>
            </div>
            <div className='flex justify-between books-center gap-x-2 mt-4 p-4'>
                <BiShow
                    className='text-3xl text-blue-500 hover:text-black cursor-pointer' onClick={()=>setShowModal(true)}
                />
